...
export class EvaluationComponent implements OnInit {
  ...
  // Buscador de pacientes — independiente del formGroup
  ...
  pacientesFiltrados = computed(() => {
    ...
    return this.pacientes()
      .filter(
        (p) => p.nombre_completo.toLowerCase().includes(q) || p.dni.includes(q),
      )
      .slice(0, 8);
  });
  ...
  readonly etiquetasFrecEjercicio = [
    'Nunca',
    'Ocasionalmente',
    'Frecuentemente',
  ];
  readonly etiquetasNivelEstres = [
    '',
    'Muy bajo',
    'Bajo',
    'Moderado',
  ...
  readonly etiquetasCalidadSueno = [
    '',
    'Muy mala',
    'Mala',
    'Buena',
  ...
  readonly etiquetasApoyoFamiliar = [
    '',
    'Muy bajo',
    'Bajo',
    'Alto',
  ...
  getEtiqueta(campo: string): string {
    const val = Number(
      this.evalForm.get(`model_features.${campo}`)?.value ?? 0,
    );
    switch (campo) {
      case 'vida_social':
        return this.etiquetasVidaSocial[val] ?? String(val);
      case 'frecuencia_ejercicio':
        return this.etiquetasFrecEjercicio[val] ?? String(val);
      case 'nivel_estres':
        return this.etiquetasNivelEstres[val] ?? String(val);
      case 'calidad_sueno':
        return this.etiquetasCalidadSueno[val] ?? String(val);
      case 'soledad_percibida':
        return this.etiquetasSoledad[val] ?? String(val);
      case 'apoyo_familiar':
        return this.etiquetasApoyoFamiliar[val] ?? String(val);
      case 'autoestima':
        return this.etiquetasAutoestima[val] ?? String(val);
      default:
        return String(val);
    }
  }
    ...
    this.evalService
      ...
      .subscribe({
        ...
        error: (err) => {
          ...
          this.alertService.error(
            'Error',
            'Hubo un problema procesando la evaluación.',
          );
          console.error(err);
        },
      });
  ...
  mostrarResultados(response: EvaluationResponse) {
    ...
    this.riesgoPorcentaje.set(
      pred?.risk_probability != null
        ? Math.round(pred.risk_probability * 100)
        : 0,
    );
  ...
  confirmarDiagnostico() {
    ...
    this.evalService.updateAgreement(evalId, 'confirmed').subscribe({
      ...
      error: () => {
        this.isSavingAgreement.set(false);
        this.alertService.error('Error', 'No se pudo registrar.');
      },
    });
  }
  ...
  confirmarDesacuerdo() {
    const reason =
      this.selectedDisagreementReason() === 'Otro'
        ? this.otherReason().trim()
        : this.selectedDisagreementReason();
    if (!reason) {
      this.alertService.error('Requerido', 'Selecciona o escribe una razón.');
      return;
    }
    const evalId = this.ultimaEvaluacion()?.id;
    if (!evalId) return;
    ...
    this.evalService.updateAgreement(evalId, 'rejected', reason).subscribe({
      next: (updated) => {
        this.doctorAgreement.set(updated.doctor_agreement ?? null);
        this.isSavingAgreement.set(false);
        this.alertService.success('Registrado', 'Desacuerdo registrado.', true);
      },
      error: () => {
        this.isSavingAgreement.set(false);
        this.alertService.error('Error', 'No se pudo registrar.');
  ...
  initShapChart(shapValues: Record<string, number> | null) {
    ...
    const entradas = shapValues
      ? Object.entries(shapValues).filter(([k]) => labelMap[k])
      : [];
    const top6 = entradas
      .sort((a, b) => Math.abs(b[1]) - Math.abs(a[1]))
      .slice(0, 6);
    const maxAbs =
      top6.length > 0 ? Math.max(...top6.map(([, v]) => Math.abs(v))) : 1;
    this.shapData = {
    ...
    this.shapOptions = {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: (ctx: any) =>
              ` ${ctx.raw >= 0 ? 'Aumenta' : 'Disminuye'} el riesgo: ${ctx.raw >= 0 ? '+' : ''}${ctx.raw}%`,
          },
        },
      },
      scales: {
  ...
  exportarPDF() {
    ...
    this.alertService.success(
      'Informe Generado',
      'El PDF se ha guardado en tus descargas.',
      true,
    );
  }

  async cerrarYVolver() {
    // 1. Si el médico YA hizo clic en "Confirmo" o "No concuerdo", cerramos de inmediato.
    if (this.doctorAgreement()) {
      this.ejecutarNavegacion();
      return;
    }

    // 2. Si se olvidó de seleccionar, lanzamos la advertencia.
    // (Tu alertService.confirm mostrará los botones de Aceptar/Cancelar por defecto)
    ...
    // 3. Si el usuario hace clic en "Aceptar"
    if (confirmado) {
      const evalId = this.ultimaEvaluacion()?.id;
      ...
      if (evalId) {
        ...
        // Registramos en base de datos que sí estuvo de acuerdo
        this.evalService.updateAgreement(evalId, 'confirmed').subscribe({
          next: () => {
            this.isSavingAgreement.set(false);
            this.alertService.close();
          ...
          error: () => {
            this.isSavingAgreement.set(false);
            this.alertService.close();
            // Si hay error igual lo dejamos salir para no bloquear su flujo
            this.ejecutarNavegacion();
          },
        });
      } else {
    ...
    // Si hace clic en "Cancelar", la variable 'confirmado' es false
    // y la función termina aquí, dejándolo en la pantalla del modal.
  }

  // Método auxiliar para no repetir la lógica de navegación
  private ejecutarNavegacion() {
    this.displayModal.set(false);
    const idPaciente = this.evalForm.get('patient_id')?.value;
    this.router.navigate(
      idPaciente ? ['/dashboard/patient', idPaciente] : ['/dashboard'],
    );
  }
}
